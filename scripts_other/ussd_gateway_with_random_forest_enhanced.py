...
============================================================
 ...
 Version Améliorée v3.0 :
   - Option 1: Simulation d'attaque par force brute (100 tentatives)
   - Option 2: Simulation de 100 transactions (50 favorables, 50 défavorables)
   - Option 3: Mode temps réel (UART)
   - Analyse IA avec Random Forest sur [Localisation, Transaction, IMEI]
   - Authentification Multifacteur (MFA) avec phrase secrète
   - Formule de Haversine pour validation localisation
============================================================
"""
...
try:
    ...
    HAS_ML = False
    print("[-] Attention: scikit-learn / numpy manquants. Moteur de risque simulé.")
    print("[-] Activation : pip install scikit-learn numpy")

NOM_FICHIER_LOG = "historique_transactions.log"
DB_FILE = "hlr.db"

# Compteur de tentatives échouées par numéro (Anti-BruteForce, en mémoire)
tentatives_echouees: dict[str, int] = {}

# Liste des Cell-IDs connus (simulés) pour la validation HLR
CELLID_CONNUS = {
    10243: "Katuba",
    20184: "Kenya Centre",
    30948: "Kamalondo",
...
# Données de localisations (latitude, longitude) pour chaque zone
LOCALISATIONS_ZONES = {
    "Katuba": (-11.2197, 27.4833),
    "Kenya Centre": (-11.1596, 27.3497),
    "Kamalondo": (-11.1897, 27.5097),
    "Kampemba": (-11.1797, 27.4597),
    "Ruashi": (-11.2497, 27.3197),
    "Gombe": (-11.1397, 27.4997),
    "Lingwala": (-11.1097, 27.5297),
    "Limete": (-11.1697, 27.4097),
    "Ngaliema": (-11.1497, 27.3797),
    "Barumbu": (-11.1297, 27.4397),
}

# Données des comptes (avec localisation habituelle et IMEI)
DONNEES_COMPTES = {
    "243970000001": {
        "nom": "MUTOMBO",
        "postnom": "NGOY",
        "pin": "1234",
        "phrase_secrete": "MSCAGNES",
        "solde": 500.0,
        "localisation_principale": "Katuba",  # Maison
        "localisation_travail": "Kenya Centre",
    ...
    "243970000002": {
        ...
        "phrase_secrete": "MSCAGNES",
        "solde": 1200.0,
        "localisation_principale": "Gombe",  # Maison
        "localisation_travail": "Lingwala",
    ...
    "243970000003": {
        ...
        "phrase_secrete": "MSCAGNES",
        "solde": 800.0,
        "localisation_principale": "Limete",  # Maison
        "localisation_travail": "Ngaliema",
...
def journaliser_activite(message: str) -> None:
    """Écrit l'activité dans le fichier de log (lu par le Dashboard)."""
...
def verifier_localisation_valide(localisation_compte: str, localisation_actuelle: str) -> int:
    """
    Vérifie si la localisation actuelle est proche des localisations habituelles du compte.
    Retourne 0 si localisation valide, 1 sinon.
    """
    if localisation_actuelle not in LOCALISATIONS_ZONES:
        return 1  # Localisation inconnue = pas de correspondance
    
    lat_actuelle, lon_actuelle = LOCALISATIONS_ZONES[localisation_actuelle]
    
    # Vérifie si c'est proche de la maison ou du travail (dans rayon de 5 km)
    for zone_habituelle in [localisation_compte]:
        if zone_habituelle in LOCALISATIONS_ZONES:
            lat_habit, lon_habit = LOCALISATIONS_ZONES[zone_habituelle]
            distance = haversine(lat_actuelle, lon_actuelle, lat_habit, lon_habit)
            if distance < 5:  # Rayon de 5 km
                return 0  # Localisation conforme
    
    return 1  # Localisation non conforme
...
class MoteurRisque:
    """
    Classifie le risque d'une transaction sur 3 niveaux :
        0 → Léger       1 → Doute (Moyen)
        2 → Très élevé
    
    Features : [localisation_mismatch, transaction_mismatch, imei_mismatch]
    (0 = conforme, 1 = non conforme)
    """
    ...
    def _generer_donnees_synthetiques(self):
        """
        Génère ~300 exemples synthétiques pour le modèle.
        Features : [localisation_mismatch (0/1), transaction_mismatch (0/1), imei_mismatch (0/1)]
        
        Règles métier :
          - 0 paramètres non conformes (0,0,0) → Léger
          - 1 paramètre non conforme → Doute
          - 2+ paramètres non conformes → Très élevé
        """
        random.seed(0)
        X, y = [], []

        def risque_attendu(loc, trans, imei):
            count_mismatch = loc + trans + imei
            if count_mismatch == 0:
                return 0  # Léger
        ...
        # Cas normaux (tout conforme) → Léger
        for _ in range(100):
            X.append([0, 0, 0])
            y.append(0)

        # Cas avec 1 paramètre non conforme → Doute
        for _ in range(100):
            loc = random.randint(0, 1)
            trans = random.randint(0, 1)
            imei = random.randint(0, 1)
            if loc + trans + imei == 1:
                X.append([loc, trans, imei])
                y.append(1)

        # Compléter avec des cas de 1 non conforme
        for _ in range(50):
        ...
        # Cas avec 2+ paramètres non conformes → Très élevé
        for _ in range(50):
            X.append([1, 1, 0])
            y.append(2)
        for _ in range(50):
    ...
    def evaluer(self, localisation_mismatch: int, transaction_mismatch: int, imei_mismatch: int) -> str:
        ...
        Chaque paramètre est 0 (conforme) ou 1 (non conforme).
        """
        if not HAS_ML or self.model is None:
            # Logique simplifiée sans ML
...
def verifier_base_donnees() -> None:
    """Crée ou vérifie l'existence de la base de données SQLite."""
    ...
    CREATE TABLE IF NOT EXISTS comptes (
        ...
        phrase_secrete TEXT,
        solde REAL,
        localisation_principale TEXT,
        localisation_travail TEXT,
        imei TEXT,
...
def simulation_force_brute(moteur_risque: MoteurRisque) -> None:
    """
    Simule 100 tentatives d'attaque par force brute sur un compte.
    Après 3 tentatives échouées, le compte est bloqué et l'attaquant doit passer par MFA.
    """
    print("\n" + "=" * 70)
    print(" SIMULATION 1 : ATTAQUE PAR FORCE BRUTE (100 tentatives)")
    print("=" * 70 + "\n")

    # Sélectionner un compte cible
    ...
    for i in range(100):
        ...
        # Générer un PIN aléatoire (90% d'échecs, 10% de réussite)
        ...
        if compte_bloque_depuis:
            ...
            # Demander MFA
            log_msg = f"[SÉCURITÉ] MFA ACTIVÉE - Demande d'authentification (nom, post-nom, phrase secrète)"
            print(log_msg)
            journaliser_activite(log_msg)
            
            # Simuler une tentative MFA (70% réussite, 30% échec)
...
# ===========================================================
#  SIMULATION 2 : 100 TRANSACTIONS (50 favorables, 50 défavorables)
# ===========================================================

def simulation_100_transactions(moteur_risque: MoteurRisque) -> None:
    """
    Simule 100 transactions (50 favorables + 50 défavorables) avec analyse IA.
    Chaque transaction analyse [Localisation, Transaction, IMEI].
    """
    print("\n" + "=" * 70)
    ...
    for transaction_num in range(1, 101):
        # Choisir aléatoirement si c'est favorable ou défavorable
        ...
        # Générer les paramètres [Localisation, Transaction, IMEI]
        if est_favorable:
            # Cas favorable : paramètres conformes
        ...
        log_msg = (f"[TRANSACTION {transaction_num}] {statut} - Risque: {niveau_risque} | "
                   f"De: {compte['nom']} {compte['postnom']} ({numero}) | "
                   f"Montant: {montant:.2f} USD | "
                   f"Localisation: {localisation_actuelle} | "
                   f"IMEI: {imei_fourni} | "
                   f"Paramètres {details}")
    ...
    log_msg = (f"[SIMULATION] Fin simulation 100 transactions : "
               f"{transactions_favorables} favorables, {transactions_defavorables} défavorables, "
               f"{bloquees_ia} bloquées par IA")
    journaliser_activite(log_msg)
...
def traiter_req_solde(parts: list[str], expediteur: dict) -> str:
    ...
    if pin_fourni != expediteur["pin"]:
        ...
        return "REPONSE;ERREUR;PIN incorrect"
...
def traiter_req_trans(parts: list[str], expediteur: dict, moteur_risque: MoteurRisque) -> str:
    """Gère REQ_TRANS;OPT:<opt>;NUM:<num>;MONT:<mont>;PIN:<pin>"""
    numero_exp = expediteur["numero_compte"]
    ...
    try:
        ...
        return "REPONSE;ERREUR;Montant invalide"

    if montant_float <= 0:
        return "REPONSE;ERREUR;Montant doit être positif"

    if expediteur["solde"] < montant_float:
        msg = f"[TRANS] ECHEC: Solde insuffisant pour {numero_exp}."
        print(msg)
        journaliser_activite(msg)
        return "REPONSE;ERREUR;Solde insuffisant"

    # Évaluer le risque avec IA
    heure = datetime.now().hour
    niveau_risque = moteur_risque.evaluer(montant_float, heure, tentatives_echouees.get(numero_exp, 0), expediteur["solde"])
    
    journaliser_activite(f"[IA] Évaluation du risque de la transaction ({montant_float:.2f} USD) : {niveau_risque}")

    # Bloquer si très élevé
    if niveau_risque == "Très élevé":
        journaliser_activite(f"[SÉCURITÉ] Alerte Fraude - Transaction bloquée")
        return "REPONSE;ERREUR;Transaction bloquée par IA"
    ...
    if not compte_dest:
        return "REPONSE;ERREUR;Compte destinataire inexistant"
...
def traiter_req_loc(parts: list[str]) -> str:
    """Gère REQ_LOC;BTS:<cellId1>:<rxLev1>,<cellId2>:<rxLev2>,..."""
    bts_str = ""
    for part in parts:
    ...
    # Vérification : au moins 1 Cell-ID doit être connu du HLR
    cellids_valides = [cid for cid in cellids_recus if cid in CELLID_CONNUS]
    if not cellids_valides:
        msg = f"[LOC] REJET: Aucun Cell-ID connu parmi {cellids_recus}. Possible BTS pirate."
        print(msg)
        journaliser_activite(msg)
        return "REP_LOC;ERREUR;Rejet HLR: BTS Invalides/Inconnues"

    zones = [CELLID_CONNUS[cid] for cid in cellids_valides]
    msg = f"[LOC] VALIDATION OK: Cell-IDs={cellids_valides} → Zones={zones}"
    print(msg)
    journaliser_activite(msg)
...
def traiter_requete_uart(ligne: str, moteur_risque: MoteurRisque) -> str:
    ...
    # --- REQUÊTES NÉCESSITANT UN ABONNÉ ---
    expediteur = get_expediteur_par_defaut()
    if not expediteur:
        return "REPONSE;ERREUR;Base de données vide ou inaccessible"
    ...
    if tentatives_echouees[numero_exp] >= 3:
        msg = f"[SÉCURITÉ] Rejet: Compte {numero_exp} COMPTE_BLOQUE (brute-force détecté)."
        print(msg)
        journaliser_activite(msg)
    ...
    try:
        if type_requete == "REQ_SOLDE":
            ...
            msg = f"[-] Trame inconnue reçue: {type_requete}"
            print(msg)
            journaliser_activite(msg)
...
# ===========================================================
#  BOUCLE PRINCIPALE (MODE TEMPS RÉEL)
# ===========================================================
...
if __name__ == "__main__":
    print("=" * 70)
    print(" PASSERELLE USSD — MOTEUR IA (Random Forest) & SQLITE v3.0")
    print(" Compatible ESP-IDF / ESP32 — Protocole UART v2.2")
    print("=" * 70)
